...
async def test_search_start(ctx: BaseContext, nhentai_ext: NHentai):
    ...
    await nhentai_search_cmd.callback(ctx, "alp love live english")

    start_msg = (await ctx.channel.fetch_messages(limit=1))[0]

    assert start_msg.content == "Select a gallery to view (Page 1 / 3)"
...
async def test_search_next_page(
    ...
    assert menu_msg.content == "Select a gallery to view (Page 2 / 3)"
